...
export default function ActivityList({ feedings, weights, eliminations, motherLogs }) {
  ...
  if (items.length === 0) {
    return <p className="text-center text-muted-foreground text-sm py-8">No events logged yet</p>;
  }
  ...
  return (
    <div className="space-y-2">
      {items.map((it, i) => {
        const Icon = it.icon;
        return (
          <div key={i} className="rounded-2xl p-3 flex items-center gap-3 bg-card border border-border">
            ...
            <div className="text-right flex-shrink-0">
              <p className="text-[10px] text-muted-foreground">{timeAgo(it.time)}</p>
              <p className="text-[10px] text-muted-foreground/60">{formatDateTime(it.time)}</p>
            </div>
          </div>
        );
      })}
