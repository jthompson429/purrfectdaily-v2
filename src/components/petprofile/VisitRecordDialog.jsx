...
function SubHeading({ children, onAdd, addLabel }) {
  return (
    <div className="flex items-center justify-between pt-1">
      <p className="text-[11px] uppercase tracking-wider text-white/50 font-bold">{children}</p>
      {onAdd && (
        <button type="button" onClick={onAdd} className="flex items-center gap-1 text-[11px] font-bold text-purple-400 hover:text-purple-300">
...
const rowInput = "bg-white/5 border-white/10 text-white rounded-xl placeholder:text-white/20 text-sm h-9";
...
export default function VisitRecordDialog({ open, onOpenChange, item, onSave }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg rounded-3xl border-white/10 bg-[#0f1117] max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={submit} className="space-y-4 mt-2">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label className="text-white/60 text-xs uppercase tracking-wider">Visit Date</Label>
              <Input type="date" value={form.date || ""} onChange={(e) => set("date", e.target.value)} className={`${inputClass} [color-scheme:dark]`} />
            </div>
            <div className="space-y-1.5">
              <Label className="text-white/60 text-xs uppercase tracking-wider">Visit Type</Label>
              <Select value={form.visit_type} onValueChange={(v) => set("visit_type", v)}>
                ...
                <SelectContent className="bg-[#0f1117] border-white/10">
                  {VISIT_TYPES.map(([v, l]) => <SelectItem key={v} value={v} className="text-white hover:bg-white/5">{l}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            <Label className="text-white/60 text-xs uppercase tracking-wider">Reason for Visit</Label>
          ...
          <div className="space-y-1.5">
            <Label className="text-white/60 text-xs uppercase tracking-wider">Treatments Performed</Label>
            ...
            {vaccs.map((gv, i) => (
              <div key={i} className={rowWrap}>
                <Select value={gv.name} onValueChange={(v) => updVacc(i, "name", v)}>
                  <SelectTrigger className="bg-white/5 border-white/10 text-white rounded-xl h-9 w-[120px]"><SelectValue /></SelectTrigger>
                  <SelectContent className="bg-[#0f1117] border-white/10">
                    {VACC_OPTIONS.map(([v, l]) => <SelectItem key={v} value={v} className="text-white hover:bg-white/5">{l}</SelectItem>)}
                  </SelectContent>
                </Select>
                ...
                <Input type="date" value={gv.due_date || ""} onChange={(e) => updVacc(i, "due_date", e.target.value)} className={`${rowInput} [color-scheme:dark] flex-1`} title="Due date" />
                <button type="button" onClick={() => rmVacc(i)} className="text-white/30 hover:text-red-400"><X className="h-4 w-4" /></button>
              </div>
            ))}
            {vaccs.length === 0 && <p className="text-[11px] text-white/30">Auto-added to the Vaccinations section when saved.</p>}
          </div>
          ...
          <div className="space-y-2 rounded-xl p-3" style={{ background: "rgba(59,130,246,0.06)", border: "1px solid rgba(59,130,246,0.18)" }}>
              ...
              <div key={i} className="space-y-2">
                <div className={rowWrap}>
                  ...
                  <Select value={gp.frequency} onValueChange={(v) => updPrev(i, "frequency", v)}>
                    <SelectTrigger className="bg-white/5 border-white/10 text-white rounded-xl h-9 w-[130px]"><SelectValue /></SelectTrigger>
                    <SelectContent className="bg-[#0f1117] border-white/10">
                      {PREV_FREQ.map(([v, l]) => <SelectItem key={v} value={v} className="text-white hover:bg-white/5">{l}</SelectItem>)}
                    </SelectContent>
                  </Select>
                  <button type="button" onClick={() => rmPrev(i)} className="text-white/30 hover:text-red-400"><X className="h-4 w-4" /></button>
                </div>
                <div className={rowWrap}>
                  <Input type="date" value={gp.date_given || ""} onChange={(e) => updPrev(i, "date_given", e.target.value)} className={`${rowInput} [color-scheme:dark] flex-1`} title="Date given" />
            ...
            {prevs.length === 0 && <p className="text-[11px] text-white/30">Auto-added to Preventive Care when saved.</p>}
              ...
              <div key={i} className="space-y-2">
                <div className={rowWrap}>
                  ...
                  <Select value={m.frequency} onValueChange={(v) => updMed(i, "frequency", v)}>
                    <SelectTrigger className="bg-white/5 border-white/10 text-white rounded-xl h-9 w-[140px]"><SelectValue /></SelectTrigger>
                    <SelectContent className="bg-[#0f1117] border-white/10">
                      {MED_FREQ.map(([v, l]) => <SelectItem key={v} value={v} className="text-white hover:bg-white/5">{l}</SelectItem>)}
                    </SelectContent>
                  </Select>
                  <button type="button" onClick={() => rmMed(i)} className="text-white/30 hover:text-red-400"><X className="h-4 w-4" /></button>
                </div>
                <div className={rowWrap}>
                  <Input type="date" value={m.start_date || ""} onChange={(e) => updMed(i, "start_date", e.target.value)} className={`${rowInput} [color-scheme:dark] flex-1`} title="Start" />
                  <Input type="date" value={m.end_date || ""} onChange={(e) => updMed(i, "end_date", e.target.value)} className={`${rowInput} [color-scheme:dark] flex-1`} title="End" />
                </div>
              </div>
            ))}
            {meds.length === 0 && <p className="text-[11px] text-white/30">You can add these to active medications from the visit card.</p>}
          </div>

          <div className="space-y-1.5">
            <Label className="text-white/60 text-xs uppercase tracking-wider">Follow-up Instructions</Label>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {atts.map((a, i) => (
                <div key={i} className="flex items-center gap-2 rounded-lg px-2.5 py-1.5" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
                  ...
                  <span className="text-xs text-white/70 max-w-[120px] truncate">{a.name}</span>
                  <button type="button" onClick={() => rmAtt(i)} className="text-white/30 hover:text-red-400"><X className="h-3.5 w-3.5" /></button>
                </div>
              ))}
              <button type="button" onClick={() => fileRef.current?.click()} className="flex items-center gap-1 text-xs font-semibold text-purple-400 hover:text-purple-300 rounded-lg px-2.5 py-1.5" style={{ background: "rgba(124,58,237,0.1)", border: "1px dashed rgba(124,58,237,0.4)" }}>
            ...
            <p className="text-[10px] text-white/25">PDF, JPG, or PNG — clinical summaries, certificates, lab results, invoices, etc.</p>
          </div>
          ...
          <DialogFooter className="pt-2 gap-2">
            ...
            <Button type="submit" disabled={saving} className="text-white rounded-xl flex-1 font-bold border-0" style={{ background: "linear-gradient(135deg, #7c3aed, #3b82f6)" }}>{saving ? "Saving…" : item?.id ? "Save Visit" : "Add Visit"}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
